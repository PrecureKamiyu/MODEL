...
def T(d2, d4):
    ...
    return [T[M-1, 1800], T[M-1, 1500]]

def binarySearch(d_list, l, r, d4):
    if r > l:
        mid = int((r + l) / 2)
        T_list = T(d_list[mid], d4)

        if T_list[0] < (47+273.15) and T_list[1] < (44+273.15):
            return binarySearch(d_list, l, mid, d4)
        else:
            return binarySearch(d_list, mid + 1, r, d4)
    else:
        return d_list[l]
    
d2_list = np.arange(0.6, 25.01, 0.1)
d4_list = np.arange(0.6, 6.5, 0.1)
...
for d4 in d4_list:
    print(i)
    best_d2 = binarySearch(d2_list, 0, len(d2_list)-1, d4)
    ans.append((d4, best_d2))
    i += 1
