...
def getAll(T1, T6, T7, T8, v):
    ...
    S = np.trapz(step_o, tspan_o) - (max_index - tL) * np.polyval(coefficients, tL)

    # 求残差
    res = 0
    for i in np.arange(217, max_temp, 1):
        # 求交点
        for t in tspan_new:
            tempL = np.polyval(coefficients, t)
            if tempL > i:
...
def optimizefunc(x):
    ...
    return getAll(20 * x[0] + 165, 20 * x[1] + 185, 20 * x[2] + 225, 20 * x[3] + 245, v_max)
