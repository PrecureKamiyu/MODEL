...
for Day in Day_list:
    ...
    for STime in ST_list:
        ...
        """
        STEP2
        calculate eta for every mirror
        """
        # monte carlo

        per_mirror_MC_times = 100
        eta_list = []
        eta_cos_list = []
        eta_sb_list = []
        eta_trunc_list = []
        ...
        for i in range(0, len(x0)):
            ...
            for j in range(0, per_mirror_MC_times):
                ...
                # check whether lambda_i is blocked by tower 
                ...
                if cos_theta < min:
                    ...
                    if x_h**2 + y_h**2 >= (D_O/2)**2 and (x - x_h)**2 + (y - y_h)**2 <= (x**2 + y**2) - (D_O/2)**2:
                        ...
                        blocked_times += 1
                        flag = 0
                # check whether lambda_o is blocked by another mirror 
                if flag == 1:
                    ...
                    if B != -1:
                        ...
                        y_Bboard = - mid_cosphi * np.sin(mid_theta) * xb_earth + \
                            mid_cosphi * np.cos(mid_theta) * yb_earth + mid_sinphi * zb_earth
                        z_Bboard = mid_sinphi * np.sin(mid_theta) * xb_earth - \
                            mid_sinphi * np.cos(mid_theta) * yb_earth + mid_cosphi * zb_earth

                        if x_Bboard >= -3 and x_Bboard <= 3 and y_Bboard >= -3 and y_Bboard <= 3:
                            flag = 0
                            blocked_times += 1
                # check whether lambda_i is blocked by another mirror 
                if flag == 1:
                    around_list = []
                    ...
                    for r in around_list:
                        m = ((x0[i] - x0[r]) * nA[0] + (y0[i] - y0[r]) * nA[1]) / \
                            (lambdai[0] * nA[0] + lambdai[1] * nA[1] + lambdai[2] * nA[2])
                        ...
                        y_Aboard = - mid_cosphi * np.sin(mid_theta) * xb_ear + \
                            mid_cosphi * np.cos(mid_theta) * yb_ear + mid_sinphi * zb_ear
                        z_Aboard = mid_sinphi * np.sin(mid_theta) * xb_ear - \
                            mid_sinphi * np.cos(mid_theta) * yb_ear + mid_cosphi * zb_ear
                        print(a)
                ...
                if flag == 1:
                    ...
                    for m in range(0, light_MC_times):
                        ...
                        y_h76 = y + ko76 * S[1]
                        min = ((x**2 + y**2) - (D_O/2)**2)**0.5 / (x**2 + y**2)**0.5
                        cos_theta = - (x * S[0] + y * S[1]) / \
                            ((x**2 + y**2)**0.5 * (S[0]**2 + S[1]**2)**0.5)
                        if cos_theta < min:
                            out_times += 1
                        else:
                            if x_h84**2 + y_h84**2 >= (D_O/2)**2 and (x - x_h84)**2 + \
                                (y - y_h84)**2 <= (x**2 + y**2) - (D_O/2)**2:
                                out_times += 1
                            else:
                                if x_h76**2 + y_h76**2 >= (D_O/2)**2 and (x - x_h76)**2 + \
                                    (y - y_h76)**2 <= (x**2 + y**2) - (D_O/2)**2:
                                    in_times += 1
                                else:
                                    out_times += 1
            ...
            """
            STEP4
            calculate other eta and light eta
            """
            cos_theta = - (lambdai[0] * n_dict[i][0] + lambdai[1] * n_dict[i][1] + \
                lambdai[2] * n_dict[i][2]) / (np.linalg.norm(lambdai) * np.linalg.norm(n_dict[i]))
            eta_cos = cos_theta
