        ...
        for i in range(0, len(x0)):
            ...
            for j in range(0, per_mirror_MC_times):
                ...
                if cos_theta < min:
                    ...
                    if x_h**2 + (y_h-yR)**2 >= (D_O/2)**2 and (x - x_h)**2 + \
                        (y - y_h)**2 <= (x**2 + (y-yR)**2) - (D_O/2)**2:
                        flag = 1
                    else:
                        blocked_times += 1
                        flag = 0
                ...
                if flag == 1:
                    ...
                    for m in range(0, light_MC_times):
                        ...
                        min = ((x**2 + (y-yR)**2) - (D_O/2)**2)**0.5 / \
                            (x**2 + (y-yR)**2)**0.5
                        cos_theta = - (x * S[0] + (y-yR) * S[1]) / \
                            ((x**2 + (y-yR)**2)**0.5 * (S[0]**2 + S[1]**2)**0.5)
                        if cos_theta < min:
                            out_times += 1
                        else:
                            if x_h84**2 + (y_h84-yR)**2 >= (D_O/2)**2 and (x - x_h84)**2 + \
                                (y - y_h84)**2 <= (x**2 + (y-yR)**2) - (D_O/2)**2:
                                out_times += 1
                            else:
                                if x_h76**2 + (y_h76-yR)**2 >= (D_O/2)**2 and (x - x_h76)**2 + \
                                    (y - y_h76)**2 <= (x**2 + (y-yR)**2) - (D_O/2)**2:
                                    in_times += 1
                                else:
                                    out_times += 1
