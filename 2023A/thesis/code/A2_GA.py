...
def min_Fun(args):
    global old_list
    global dot_list
    yR, a, b, d, delta_r = args
    if (dot_list == []):
        generate_dotlist(yR, delta_r, a)
        old_list = [yR, a, b, d, delta_r]
    else:
        if old_list[0:5] != [yR, a, b, d, delta_r]:
            generate_dotlist(yR, delta_r, a)
            old_list[0:5] = [yR, a, b, d, delta_r]
...
ga = GA(func=min_Fun, n_dim=5,    prob_mut=0.01,    size_pop=100,   max_iter=1000,
        lb=[-350, 6, 2, 2, 10], ub=[0, 8, 6, 6, 20], constraint_eq=[eq_Efield])
