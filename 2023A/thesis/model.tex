...
%
从该图本文可得出如下结论：
问题1中所给的镜面是在以吸收塔为圆心的多个同心圆上，且在每一个圆呈均匀排布的。通过进一步分析可以得出相邻两个同心圆之间的半径差值也是相等的，其值为13.484\(\,\mathrm{m}\)。
%%
\paragraph{计算法向量}
% TODO rearange require
针对问题中的任意一个日期\(D\)和时间点\(ST\)，由模型准备之中太阳高度角、方位角的相关公式可以确定在此时刻的太阳高度角和太阳方位角，于是可以确定太阳中心发出的光线，也即定日镜的入射光线的方向向量\(\vec \lambda_i=(x_i,y_i,z_i)\)。
...
由于定日镜场的地理位置位于北纬\(39.4^{\circ}\)，太阳光线在一年中的沿南北方向的分量一定是由南向北的，因此在地面坐标系\(xyz\)下定日镜\(A\)只能被南面的定日镜挡住光线。本文通过分析定日镜场每一圈之间的间隙长度\(d_{\mathrm{belt}}\)，以及图~(\ref{arange_1})中场内定日镜的分布和相邻定日镜之间的排布间隙要比镜面宽度\(a\)至少大\(5 \, \mathrm{m}\)的要求，
...
\]
联立上面直线方程和平面方程即可得到定日镜\(B\)的端点在定日镜A镜面上的投影点 \(B' (x_{B}' , y_{B} ' , z _{B} ')\)。由于得到的坐标是地面坐标系下的坐标，还需使用 equ~(\ref{transM}) 将其转化为\(A\)镜面系下的坐标：
\[
\begin{pmatrix}
x_{B} ''\\
y_{B} ''\\
