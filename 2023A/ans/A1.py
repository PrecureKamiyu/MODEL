import numpy as np
import pandas as pd
import random

"""
STEP1
"""

data = pd.read_excel('附件.xlsx', sheet_name= 'Sheet1')

# 中心坐标
x0 = []
y0 = []
for row in data.iloc:
    x0.append(row[0])
    y0.append(row[1])

h = 80
h_collect = 8
d = 4
D_O = 7
mid = h - h_collect / 2
xR = 0
yR = 0
a = 6
b = 6
eta_ref = 0.92
# 建立一个面板字典，key为面板编号，存储  从中心到反射点的向量  信息

mirror = {}
for i in range(0, len(x0)):
    n = [x0[i] - xR, y0[i] - yR, d - mid]
    n = n / np.linalg.norm(n)
    mirror[i] = n

# 根据半径分类第几排
mirror_R = {}

for i in range(0, len(x0)):

    mirror_R[i] = round((x0[i]**2 + y0[i]**2)**0.5)

# 获得special间断点列表
special = [0]
front = 108
for i in range(0, len(x0)):
    if mirror_R[i] == front:
        continue
    else:
        front = mirror_R[i]
        special.append(i)
special.append(len(x0))

# 判断并返回前排列表函数
def check_and_return_front(check):
    for i in range(1, len(special)):
        if check < special[i] and i == 1:
            return []
        elif check < special[i]:
            return range(special[i - 2], special[i - 1])


Day_list = [305, 336, 0, 30, 60, 91, 121, 152, 183, 213, 244, 274]
ST_list = [9, 10.5, 12, 13.5, 15]

# 以下写某两组个循环
# 先以3.21的9点为例

# for i in Day_list:
#     for j in ST_list:
D = Day_list[0]
ST = ST_list[0]

phi = np.deg2rad(39.4)
omega = np.pi / 12 * (ST - 12)
sindelta = np.sin(2 * np.pi * D / 365) * np.sin(2 * np.pi * 23.45 / 360)

cosdelta = (1 - sindelta**2)**0.5
# alphas
sinalphas = cosdelta * np.cos(phi) * np.cos(omega) + sindelta * np.sin(phi)
cosalphas = (1 - sinalphas**2)**0.5
# gammas
cosgammas = (sindelta - sinalphas * np.sin(phi))/(cosalphas * np.cos(phi))
singammas = np.sign(ST - 12) * (1 - cosgammas**2)**0.5

# 确定所有面板的法向量

xn = - cosalphas * singammas
yn = - cosalphas * cosgammas
zn = - sinalphas

# 入射光线向量
lambdai = (xn, yn, zn)

# 法向量字典
n_dict = {}
for i in range(0, len(x0)):
    n_dict[i] = - (mirror[i] + n)

"""
STEP2
"""
# 开始在all面板中monte carlo


per_mirror_MC_times = 100
eta_list = []
eta_cos_list = []
eta_sb_list = []
eta_trunc_list = []


# 面板进行循环计算其各种效率
for i in range(0, len(x0)):
    block_times = 0
    eta_dot_trunc_list = []
    for j in range(0, per_mirror_MC_times):
        # 生成随机点
        xA = random.uniform(-a/2, a/2)
        yA = random.uniform(-b/2, b/2)
        nA = n_dict[i]

        # 求最近前板编号B
        front_list = check_and_return_front(i)
        dAB = np.inf
        B = 0
        for k in front_list:
            dAB_temp = ((x0[i] - x0[k])**2 + (y0[i] - y0[k])**2)**0.5
            if dAB_temp <= dAB:
                dAB_temp = dAB
                B = k
        
        # 转换A点的坐标
        x_hat = (nA[1] / (nA[0]**2 + nA[1]**2)**0.5, nA[0] / (nA[0]**2 + nA[1]**2)**0.5, 0)
        y_hat = (-nA[0] / (nA[0]**2 + nA[1]**2)**0.5, nA[1] / (nA[0]**2 + nA[1]**2)**0.5, -(nA[0]**2 + nA[1]**2)**0.5 / nA[2])
        y_hat = y_hat / np.linalg.norm(y_hat)

        x= x0[i] + x_hat[0] * xA + y_hat[0] * yA
        y= y0[i] + x_hat[1] * xA + y_hat[1] * yA
        z= d + y_hat[2] * yA
        
        # 先判断入射光线是否经过圆柱塔
        # 比例系数k   入射光线与平面 z = h 的交点 Nh
        k = (h - z) / lambdai[2]
        x_h = x + lambdai[0] * k
        y_h = y + lambdai[1] * k

        # 先计算入射光线与中心线AO在xOy维度内的夹角
        # 最小余弦值计算
        min = ((x**2 + y**2) - (D_O/2)**2)**0.5 / (x**2 + y**2)**0.5

        cos_theta = (x * lambdai[0] + y * lambdai[1]) / ((x**2 + y**2)**0.5 * (lambdai[0]**2 + lambdai[1]**2)**0.5)

        # flag 为是否计算trunc的标志变量
        flag = 1
        if cos_theta < min:
            flag = 1
        else:
            if x**2 + y**2 >= (D_O/2)**2 and (x - x_h)**2 + (y - y_h)**2 <= (x**2 + y**2) - (D_O/2)**2:
                flag = 1
            else:
                block_times += 1
                flag = 0

        
        # 接下来判断是否被B板挡住入射光（或者反射光）
        pass

        # 接下来计算trunc
        if flag == 1:
            
            # 根据A点坐标，确定反射光线，利用光锥,算该点的截断效率
            # eta_dot_trunc_list.append(trunc)
            pass

    eta_sb = block_times / per_mirror_MC_times
    if len(eta_dot_trunc_list) == 0:
        eta_trunc = 0
    else:
        eta_trunc = sum(eta_dot_trunc_list) / len(eta_dot_trunc_list)
    
    # 通过法向量和入射光线向量确定eta_cos
    sin_theta = - (lambdai[0] * n_dict[i][0] + lambdai[1] * n_dict[i][1] + lambdai[2] * n_dict[i][2]) / (np.linalg.norm(lambdai) * np.linalg.norm(n_dict[i]))
    cos_theta = (1 - sin_theta**2) ** 0.5
    eta_cos = 1 - cos_theta


    d_HR = ((x0[i] - xR)**2 + (y0[i] - yR)**2 + (d - mid)**2)**0.5
    eta_at = 0.99321 - 0.0001176 * d_HR + 1.97 * (1e-8) * d_HR ** 2

    eta = eta_sb * eta_trunc * eta_cos * eta_at * eta_ref


    eta_list.append(eta)
    eta_cos_list.append(eta_cos)
    eta_sb_list.append(eta_sb)
    eta_trunc_list.append(eta_trunc)

avg_eta = sum(eta_list) / len(x0)
avg_eta_cos = sum(eta_cos_list)  / len(x0)
avg_eta_sb = sum(eta_sb_list) / len(x0)
avg_eta_trunc = sum(eta_trunc_list) / len(x0)


G0 = 1.366
H = 3
a = 0.4237 - 0.00821 * (6 - H)**2
b = 0.5055 + 0.00595 * (6.5 - H)**2
c = 0.2711 + 0.01858 * (2.5 - H)**2

DNI = G0 * (a + b * np.exp(-c / sinalphas))

# 这里因为所有定日镜面积一致   简略
E_field = DNI * a * b * sum(eta_list)